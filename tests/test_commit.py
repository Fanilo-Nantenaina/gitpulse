...
def test_collect_all_scope_includes_staged_and_untracked(dirty_repo: Path) -> None:
...
def test_collect_staged_scope_only_index(dirty_repo: Path) -> None:
    wc = collect_working_changes(dirty_repo, scope="staged")
    paths = {f.path for f in wc.files}
    assert paths == {"app.py"}


def test_collect_tracks_line_counts(dirty_repo: Path) -> None:
    wc = collect_working_changes(dirty_repo, scope="staged")
...
def test_commit_message_local_fallback(dirty_repo: Path) -> None:
    wc = collect_working_changes(dirty_repo, scope="all")
