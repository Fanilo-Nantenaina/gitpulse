...
def test_newline_in_path_is_rejected(
    generator: Callable[[str, str, str, str], tuple[str, str, str]],
) -> None:
    with pytest.raises(U.UnitValueError):
        generator(NEWLINE_INJECTION, "1d", "7d", "slack")


def test_plist_stays_valid_xml_with_hostile_path() -> None:
    _, plist, _ = U.launchd_watch('/repo/<*>&"q"', "1d", "7d", "slack")
    xml.dom.minidom.parseString(plist)
    assert "&lt;*&gt;&amp;" in plist


def test_quote_in_windows_path_is_rejected_not_mangled() -> None:
    with pytest.raises(U.UnitValueError, match="double quote"):
        U.windows_watch('C:\\r" /TR "calc.exe', "1d", "7d", "slack")


def test_percent_is_escaped_for_systemd_specifiers() -> None:
...
def test_good_every_is_parsed(every: str, expected: tuple[int, str]) -> None:
    assert U.validate_every(every) == expected


@pytest.mark.parametrize("to", ["", ",", "slack;curl", "sl ack", "-bad"])
def test_bad_channels_are_rejected(to: str) -> None:
    with pytest.raises(U.UnitValueError):
        U.validate_channels(to)


def test_channels_are_split_and_trimmed() -> None:
