...
def test_loopback_host_header_is_accepted(client, host):
    assert client.get("/api/config", headers={"Host": host}).status_code == 200


def test_operator_can_allow_an_extra_host(monkeypatch):
...
def test_bind_host_is_allowed(monkeypatch):
...
def test_cross_origin_read_is_rejected(client):
    r = client.get("/api/browse", headers={"Origin": "https://evil.test"})
    assert r.status_code == 403


def test_opaque_origin_is_rejected(client):
...
def test_same_origin_page_request_is_allowed(client):
    r = client.post(
        "/api/config/lang",
        json={"lang": "fr"},
...
def test_originless_local_client_is_allowed(client):
    assert client.get("/api/config").status_code == 200


def test_index_is_still_served(client):
