...
def test_cache_is_populated_by_a_collection(linear_repo):
...
def test_second_collection_computes_no_diffs(linear_repo):
...
def test_cached_and_uncached_results_are_identical(linear_repo):
    def snapshot(a):
        return [
...
def test_caller_cannot_corrupt_a_cache_entry(linear_repo):
...
def test_cache_is_keyed_per_repository(linear_repo, branched_repo):
    collect_activity(linear_repo, SINCE)
...
def test_cache_is_bounded(linear_repo, monkeypatch):
    monkeypatch.setattr(collector, "_DIFF_CACHE_MAX", 2)
    collect_activity(linear_repo, SINCE)
    assert collector.diff_cache_info()["entries"] <= 2


def test_clear_diff_cache_empties_it(linear_repo):
    collect_activity(linear_repo, SINCE)
    collector.clear_diff_cache()
    assert collector.diff_cache_info()["entries"] == 0
...
def test_summary_response_carries_stats(client, linear_repo):
    r = client.post(
        "/api/summary",
...
def test_inline_stats_match_the_dedicated_endpoint(client, linear_repo):
...
def test_summary_collects_the_window_only_once(client, linear_repo):
    body = {"path": str(linear_repo), "when": "all", "provider": "local"}
    with mock.patch(
